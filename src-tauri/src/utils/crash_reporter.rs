...
pub fn init_crash_reporting(log_path: PathBuf) {
    info!("Crash reporting initialized at: {:?}", log_path);
    *CRASH_LOG_PATH.lock().unwrap() = Some(log_path);
}

pub fn log_crash(message: &str, location: &str, line: u32, additional_context: Option<&str>) {
    let mut crash_entry = format!("[CRASH] {} at {}:{}\n", message, location, line);
    if let Some(context) = additional_context {
        crash_entry.push_str(&format!("Context: {}\n", context));
    }
    ...
    if let Some(log_path) = CRASH_LOG_PATH.lock().unwrap().as_ref() {
        if let Ok(mut file) = OpenOptions::new().create(true).append(true).open(log_path) {
            ...
            error!("Failed to write to crash log file");
        }
    }
}
...
pub fn report_panic(panic_info: &std::panic::PanicHookInfo) {
    ...
    let additional_context = format!("Thread: {}", thread_name);

    log_crash(message, location.file(), location.line(), Some(&additional_context));
}
